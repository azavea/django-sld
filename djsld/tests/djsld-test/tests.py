...
class QuantileTest(unittest.TestCase):
    def setUp(self):
        ...
        for x in range(0,5):
            ...
            p = Pipeline(material='ceramic', diameter=x, path=GEOSGeometry('LINESTRING(%d %d, %d %d)'%(x,x,x+1,x+1,)))
            p.save()
        ...
        for y in range(0,50):
            ...
            p = Pipeline(material='concrete', diameter=y, path=GEOSGeometry('LINESTRING(%d %d, %d %d)'%(x,x,x+1,x+1,)))
            p.save()
    ...
    def test_ei_classes_ln(self):
        sld = generator.as_equal_interval(Pipeline.objects.filter(material='ceramic'), 'diameter', 5)

        self.assertEqual(len(sld.NamedLayer.UserStyle.FeatureTypeStyle.Rules), 5)

        sld = generator.as_equal_interval(Pipeline.objects.filter(material='concrete'), 'diameter', 5)
    ...
    def test_ei_classes_poly(self):
        sld = generator.as_equal_interval(Reservoir.objects.filter(name__startswith='City'), 'volume', 5)

        self.assertEqual(len(sld.NamedLayer.UserStyle.FeatureTypeStyle.Rules), 5)

        sld = generator.as_equal_interval(Reservoir.objects.filter(name__startswith='County'), 'volume', 5)
    ...
    def test_fj_classes_ln(self):
        sld = generator.as_fisher_jenks(Pipeline.objects.filter(material='ceramic'), 'diameter', 5)

        self.assertEqual(len(sld.NamedLayer.UserStyle.FeatureTypeStyle.Rules), 5)

        sld = generator.as_fisher_jenks(Pipeline.objects.filter(material='concrete'), 'diameter', 5)
    ...
    def test_fj_classes_poly(self):
        sld = generator.as_fisher_jenks(Reservoir.objects.filter(name__startswith='City'), 'volume', 5)

        self.assertEqual(len(sld.NamedLayer.UserStyle.FeatureTypeStyle.Rules), 5)

        sld = generator.as_fisher_jenks(Reservoir.objects.filter(name__startswith='County'), 'volume', 5)
    ...
    def test_jc_classes_ln(self):
        sld = generator.as_jenks_caspall(Pipeline.objects.filter(material='ceramic'), 'diameter', 5)

        self.assertEqual(len(sld.NamedLayer.UserStyle.FeatureTypeStyle.Rules), 5)

        sld = generator.as_jenks_caspall(Pipeline.objects.filter(material='concrete'), 'diameter', 5)
    ...
    def test_jc_classes_poly(self):
        sld = generator.as_jenks_caspall(Reservoir.objects.filter(name__startswith='City'), 'volume', 5)

        self.assertEqual(len(sld.NamedLayer.UserStyle.FeatureTypeStyle.Rules), 5)

        sld = generator.as_jenks_caspall(Reservoir.objects.filter(name__startswith='County'), 'volume', 5)
    ...
    def test_jcf_classes_ln(self):
        sld = generator.as_jenks_caspall_forced(Pipeline.objects.filter(material='ceramic'), 'diameter', 5)

        self.assertEqual(len(sld.NamedLayer.UserStyle.FeatureTypeStyle.Rules), 5)

        sld = generator.as_jenks_caspall_forced(Pipeline.objects.filter(material='concrete'), 'diameter', 5)
    ...
    def test_jcf_classes_poly(self):
        sld = generator.as_jenks_caspall_forced(Reservoir.objects.filter(name__startswith='City'), 'volume', 5)

        self.assertEqual(len(sld.NamedLayer.UserStyle.FeatureTypeStyle.Rules), 5)

        sld = generator.as_jenks_caspall_forced(Reservoir.objects.filter(name__startswith='County'), 'volume', 5)
    ...
    def test_mb_classes_ln(self):
        sld = generator.as_maximum_breaks(Pipeline.objects.filter(material='ceramic'), 'diameter', 5)
   
        # request 5, get 2 back.
        self.assertEqual(len(sld.NamedLayer.UserStyle.FeatureTypeStyle.Rules), 2)

        sld = generator.as_maximum_breaks(Pipeline.objects.filter(material='concrete'), 'diameter', 5)
    ...
    def test_mb_classes_poly(self):
        sld = generator.as_maximum_breaks(Reservoir.objects.filter(name__startswith='City'), 'volume', 5)
   
        # request 5, get 2 back.
        self.assertEqual(len(sld.NamedLayer.UserStyle.FeatureTypeStyle.Rules), 2)

        sld = generator.as_maximum_breaks(Reservoir.objects.filter(name__startswith='County'), 'volume', 5)
    ...
    def test_nb_classes_ln(self):
        sld = generator.as_natural_breaks(Pipeline.objects.filter(material='ceramic'), 'diameter', 5)

        self.assertEqual(len(sld.NamedLayer.UserStyle.FeatureTypeStyle.Rules), 5)

        sld = generator.as_natural_breaks(Pipeline.objects.filter(material='concrete'), 'diameter', 5)
    ...
    def test_nb_classes_poly(self):
        sld = generator.as_natural_breaks(Reservoir.objects.filter(name__startswith='City'), 'volume', 5)

        self.assertEqual(len(sld.NamedLayer.UserStyle.FeatureTypeStyle.Rules), 5)

        sld = generator.as_natural_breaks(Reservoir.objects.filter(name__startswith='County'), 'volume', 5)
    ...
    def test_q_classes_ln(self):
        sld = generator.as_quantiles(Pipeline.objects.filter(material='ceramic'), 'diameter', 5)

        self.assertEqual(len(sld.NamedLayer.UserStyle.FeatureTypeStyle.Rules), 5)

        sld = generator.as_quantiles(Pipeline.objects.filter(material='concrete'), 'diameter', 5)
    ...
    def test_q_classes_poly(self):
        sld = generator.as_quantiles(Reservoir.objects.filter(name__startswith='City'), 'volume', 5)

        self.assertEqual(len(sld.NamedLayer.UserStyle.FeatureTypeStyle.Rules), 5)

        sld = generator.as_quantiles(Reservoir.objects.filter(name__startswith='County'), 'volume', 5)
